...
{   
    ...
    // Movernos a la derecha
    // Título
    $this->Cell(0,20,'Productos que generan menos ingreso de dinero',0,0,'C');//titulo documento
...
$pdf->Ln(10);
$pdf->Cell(80,10,utf8_decode('Producto'),1,0);
$pdf->Cell(60,10,utf8_decode('Cantidad'),1,0);
$pdf->Cell(60,10,utf8_decode('Total'),1,0);
$pdf->Ln(10);
$pdf->SetFont('Arial','',8);
include '../login/conexion.php'; 
                            $sql="select productos.nombre,COUNT(detalle_pedido.producto),SUM(detalle_pedido.cantidad*productos.precio_v) as Compras from detalle_pedido,productos where detalle_pedido.producto=productos.id GROUP by detalle_pedido.producto order by Compras asc limit 10";
                            $consulta=mysql_query($sql,$conexion) or die ("error ".mysql_error());
                            $numRegistros=mysql_num_rows($consulta);
                            if($numRegistros>0) {
                            while($row=mysql_fetch_array($consulta)){
                                $pdf->Cell(80,10,utf8_decode($row[0]),1,0);
                                $pdf->Cell(60,10,utf8_decode($row[1]),1,0);
