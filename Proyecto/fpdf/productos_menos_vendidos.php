...
{   
    ...
    // Movernos a la derecha
    // Título
    $this->Cell(0,20,'Productos menos vendidos',0,0,'C');//titulo documento
...
$pdf->SetFont('Arial','',8);
include '../login/conexion.php'; 
                            $sql="select productos.nombre,COUNT(detalle_pedido.producto) as cantidad,SUM(detalle_pedido.cantidad*productos.precio_v) as Compras from detalle_pedido,productos where detalle_pedido.producto=productos.id GROUP by detalle_pedido.producto order by cantidad asc limit 10";
                            $consulta=mysql_query($sql,$conexion) or die ("error ".mysql_error());
                            $numRegistros=mysql_num_rows($consulta);
                            if($numRegistros>0) {
                            while($row=mysql_fetch_array($consulta)){
