...
{   
    ...
    // Movernos a la derecha
    // Título
    $this->Cell(0,20,'Clientes frecuentes',0,0,'C');//titulo documento
...
$pdf->Ln(10);
$pdf->Cell(80,10,utf8_decode('Cliente'),1,0);
$pdf->Cell(60,10,utf8_decode('Cantidad de compras'),1,0);
$pdf->Cell(60,10,utf8_decode('Total $ comprado'),1,0);
$pdf->Ln(10);
$pdf->SetFont('Arial','',8);
include '../login/conexion.php'; 
                            $sql="SELECT usuario.usuario,COUNT(pedido.usuario),SUM(pedido.total) as Compras from usuario,pedido WHERE usuario.usuario=pedido.usuario GROUP by usuario.usuario ORDER by Compras DESC";
                            $consulta=mysql_query($sql,$conexion) or die ("error ".mysql_error());
                            $numRegistros=mysql_num_rows($consulta);
                            if($numRegistros>0) {
                            while($row=mysql_fetch_array($consulta)){
